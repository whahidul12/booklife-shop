...
interface PublishersTableProps {
  ...
  onViewPublisher: (publisher: Publisher) => void;
...
export function PublishersTable({
  ...
  const allSelected = publishers.length > 0 && selectedIds.length === publishers.length;
  const isIndeterminate = selectedIds.length > 0 && selectedIds.length < publishers.length;
        ...
        <table className="w-full min-w-[700px] text-left text-xs">
          ...
          <thead className="bg-[#fcfdfe] border-b border-gray-100 text-gray-500 font-medium select-none">
            <tr>
              ...
              <th className="px-4 py-4 font-semibold text-gray-600">Publisher</th>
              <th className="px-4 py-4 font-semibold text-gray-600">Publisher ID</th>
              <th className="px-4 py-4 font-semibold text-gray-600">Logo Status</th>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-4 py-16 text-center">
                  ...
                  <h4 className="text-sm font-bold text-gray-800">No publishers found</h4>
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 text-center">
                      <input
                        type="checkbox"
                        checked={isSelected}
                        onChange={() => onToggleSelectRow(pub.id)}
                        className="size-4 rounded border-gray-300 text-[#D10A13] focus:ring-[#D10A13] accent-[#D10A13] cursor-pointer"
                      />
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-3">
                        <div
                          onClick={() => onViewPublisher(pub)}
                          className="relative size-10 shrink-0 cursor-pointer overflow-hidden rounded-xl border border-gray-200/80 bg-white p-1 shadow-2xs transition-transform hover:scale-105 flex items-center justify-center"
                        >
                          {pub.logoUrl ? (
                            <Image
                              src={pub.logoUrl}
                    ...
                    <td className="px-4 py-3.5">
                      {pub.logoUrl ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-0.5 text-[11px] font-semibold text-emerald-700 border border-emerald-100">
                          <ImageIcon className="size-3" /> Branded
                        </span>
                      ) : (
                    ...
                    <td className="px-4 py-3.5 text-center">
                      <PublisherActionMenu
                        publisher={pub}
                        onView={onViewPublisher}
                        onEdit={onEditPublisher}
