...
import { deletePublisherAction } from "@/features/books/actions/taxonomy.actions";
...
export function PublisherDeleteModal({
  ...
  async function handleDelete() {
    setIsDeleting(true);
    try {
      const res = await deletePublisherAction(publisher.id);
      if (res.error) {
        onError(res.error);
        setIsDeleting(false);
      } else {
        onDeleted(publisher.name);
        onClose();
      }
    } catch {
      onError("Failed to delete publisher");
      setIsDeleting(false);
    }
  }
    ...
    >
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl bg-white p-6 shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center gap-3.5 mb-4">
          ...
          <div>
            <h3 className="text-base font-bold text-gray-900">Delete Publisher</h3>
        ...
        <p className="text-xs text-gray-600 leading-relaxed mb-6">
          ...
          <strong className="text-gray-900 font-semibold">"{publisher.name}"</strong>?
          Books referencing this publisher will have their publisher association cleared.
        </p>
